...
### 추상화의 TradeOff
- DI, OCP 원칙을 지키는 것은 중요하다. 해당 프로젝트의 예제에서는 Service가 구성하는 ItemRepository의 구조를 무너뜨리지 않기 위해서 ItemRepository를 구현하는 Spring Data JPA 어댑터를 만들어서 DI, OCP를 지킬 수 있도록 하였다. 그러나 해당 구조는 실제로 실무에 적용해보면 상당히 복잡하고, 이런 어댑터들도 따로 개발을 해주어야 하기 때문에 추상화에 드는 비용이 증가한다.   
따라서 프로젝트에 따라서 DI, OCP를 철저히 지킬건지, DI, OCP를 지키지 않고 직관적으로 사용할 수 있도록 구조를 간단화 할건지에 대해서 잘 정해주어야 하는데, 이런 Trade-Off에 대해서 생각하는 것이 중요하다.
> 여기서부터는 내 생각이다. 개발을 하다보면 새 기술을 도입해야 하는 상황이 있을 수도 있고, 때로는 이렇게 구현하는 것으로 충분할 줄 알았는데, 실제로 JdbcTemplate를 사용해서 해결할 수 있는 복잡한 쿼리가 존재 할 수 있다. 이런 경우를 대비해서 추상화를 해두는 것이 맞다고 생각한다. 이런 상황이 발생하면 안되겠지만 추상화를 하지 않았을 때, 기존에 구현해놓았던 구조를 무너뜨려야 하는 경우에는 구조를 무너뜨리고 새로이 구현하는데에 드는 비용 또한 만만치 않기 때문에, 더 앞을 내다보고 구조를 지키면서 개발하는 것이 미래에 더 큰 이득을 안겨줄 수 있는 상황에 빠르게 대처 할 수 있다고 생각한다.  

### 트랜잭션 매니저의 선택
- 일반적으로 실무에서 작업을 할 때 JPA를 사용하게 되면 JPA, Spring Data JPA, QueryDsL 을 사용하게 된다. 이때, 필요한 트랜잭션 매니저는 JpaTransactionManager 이다. 그러나 복잡한 쿼리의 경우 JdbcTemplate나 MyBatis를 사용해야 될 때도 있는데, 이럴 경우에는 DataSourceTransactionManger를 사용해야 될 것 이다. 따라서 서로의 트랜잭션이 따로 관리되는 문제가 발생하게 되는데, 다행 스럽게도 JpaTransactionManager는 DataSourceTransactionManager가 지원하는 기능의 대부분을 지원하기때문에 JpaTransactionManager를 사용하게 되면 트랜잭션이 따로 동작하는 문제가 발생하지 않는다. 그러나 JPA는 데이터를 변경하면 즉시 데이터를 DB에 반영하지 않고, 영속성 컨텍스트에 보관한다. 이럴 경우 MyBatis, JdbcTemplate가 변경 된 데이터를 사용하지 못하는 문제가 생기는데, 이런 경우에는 JPA의 작업이 끝난 경우에는 `em.flush()` 를 통해 영속성 컨텍스트에 있는 데이터를 DB에 반영해주어야 한다. 이때도, 트랜잭션이 정상적으로 수행되지 않으면 롤백이 가능하기 때문에, 원자성을 지킬 수 있다.
