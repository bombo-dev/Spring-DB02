...
### ORM이란
- ORM(Object-Relational-Mapping)은 용어 그대로 객체와 관계형 DB간에 매핑을 도와준다.
따라서 ORM은 JAVA 애플리케이션과 JDBC 그 사이에서 매핑을 해주고 해당 변환된 매핑을 토대로 JDBC가 DB로 쿼리를 날린다. 가지고 올 때에도 마찬가지로 JDBC에서 ORM을 거쳐서 객체가 반환이 된다.
- 많은 패러다임 불일치와, 쿼리의 단순화, 캐시의 사용으로 인한 작은 부분의 최적화와 특히, 지연로딩의 장점으로 인해 애플리케이션의 성능을 최적화 할 수 있다.

### ORM vs Mapper
- ORM의 가장 큰 장점은 아무래도 SQL에 종속된 개발을 하지 않아도 된다는 것이다. 기존에 SQL Mapper를 사용하게 되면 추가로 파라미터도 다시 추가해주어야 하고, SQL도 수정해야 하는 문제가 있었지만, ORM을 사용하면 객체 기반으로 매핑을 해주기때문에 크게 수정을 해줄 필요가 없다는 것이다. 그렇다고 SQL을 몰라도 된다는 것은 아니다. 실제 JPA가 호출하는 쿼리가 어떤 쿼리가 호출 될 지에 대해서 알고는 있어야 한다. 그래야 예측을 하고 성능을 개선할 수 있다. 또 추가적으로 Mapper의 문제점은 SQL상에 문제가 있거나 할 때 결국 실행을 해서 런타임 단계에서 확인을 해야하는 문제가 있었지만, ORM에서 사용가능한 Querydsl 라이브러리를 이용하면 컴파일 단계에서 오류를 잡아낼 수 있기때문에 개발 편의성을 높여준다.

### JPA 적용과 참고사항
- JPA는 SpringBoot의 spring-data-jpa 라이브러리를 설치하면 자동으로 된다. 이 때, 놓쳤던 부분이 MyBatis와 JPA를 추가하면 알아서 jdbc도 추가되니 라이브러리에서 제거해도 문제없다고 한다. 또한 처음에 JPA를 배울 때 많이 당황했던 부분이 있었는데, 로그가 2번 찍히는 문제가 있었다. logging의 hibernate.sql 뿐만 아니라 spring.jpa.show-sql 도 사용했기 때문인데, 후자는 System.out 으로 나오는 것이기 때문에 권장하지 않는다. 따라서 로그를 확인하기 위해서는 logging만 사용하자.
...
- 기존 프로젝트를 진행하면서 Spring Data JPA 를 사용했는데, Spring Data JPA 라는 기술 표준이 있는 걸로 알고 있었다. 오로지 JPA의 쿼리를 간편하게 짜고, 페이징 처리 같은 것을 편리하게 하기 위한 도구인 줄 알았는데, 그것보다 더 크게 Spring Data 라는 기술이 존재했고, 해당 기술은 다른 데이터베이스 NoSQL, SQL, Redis에 상관없이 해당 DB들이 하는 작업들은 비슷하고, 이를 인터페이스를 통한 구현체를 만듦으로서 편리하게 사용하기 위해 제작된 것이었다. Spring Data JPA는 Spring Data에 속한 어떠한 기술 중에 하나였던 것이다.   
추가적으로 Spring Data 기술을 사용하기 위해서는 적어도 사용할 기술에 대해 알고 있어야 한다. 모르는 상태로 사용하는 것도 물론 가능하지만, 제대로 사용하지 못할 것이고, 거의 자바를 모른채 스프링을 사용하는 것과 같다고 한다.

### Spring Data JPA 구현
- Spring Data JPA의 구현이 궁금했는데, 이 부분에 대해서 해소할 수 있었다. JpaRepository를 통해서 인터페이스를 상속받고, 상속받은 인터페이스에 대한 구현체는 생성하지 않았음에도 불구하고, JpaReposiotry가 제공하는 기능들 혹은 Spring Data JPA가 상속받은 Spring Data의 CrudRepository의 기능을 사용할 수 있었다. 그 이유는 또 프록시 패턴에 있었다. 인터페이스에 Spring Data를 적용하고 나면, 프록시 패턴을 통해 자동으로 구현체를 만들어주는 것이었다. 따라서 우리는 생성된 프록시 객체를 통해 쿼리 메서드를 실행하는 것이였다. 추가적으로 클린코드를 공부하면서 메서드 이름이 길더라도 사람이 보고 바로 이해할 수 있는 메서드 명을 선택하는 것이 좋다고 해서 실제로 긴 이름의 메서드를 사용했는데, 이를 권장하지 않는다고 한다. 실제로 긴 내용의 쿼리 같은 경우는 이름을 짧게 쓰고 Query를 작성하고 주석이나 Java Docs를 사용하는게 좋다고 한다. 

# 2023-03-17

### itemService 의존관계
- 기존에 itemService 로직은 itemRepository 인터페이스에 의존하고 있다. SpringDataJPA를 의존주입 하기 위해서 새로 추가하면 서비스 로직을 전부다 바꿔줘야 하는 대참사가 발생한다. 이를 위해 itemRepositoryV2 를 만들고, itemRepositoryV2에서 SpringDataJPA의 인터페이스를 구성하고, 빈 등록시에 프록시로 생성된 구현체로 구성하면 서비스 로직을 수정하지 않고도 정상 수행 되는 것을 볼 수 있었다. 이는 디자인패턴에서 전략패턴과 같다. 이와 같이 설정과 구현을 따로 나눠서 개발을 할 수 있도록 추상화를 잘해야 한다.
...
### QueryDSL의 용도
- SQL 혹은 JPQL 같은 경우 쿼리를 작성할 떄 문자이기 때문에 오류가 발생할 가능성이 크다. SQL Mapper는 xml에 작성해서 쉽게 구분 할 수 있지만, 다른 건 그렇지 않다.
이러한 문제를 해결해주기 위해서 QueryDSL은 쿼리를 Java로 type-safe하게 개발할 수 있도록 한다. 기존에는 문자이기때문에 테스트 케이스를 작성하거나, 직접 실행해봐야 알지만, QueryDSL은 쿼리에 있는 문제를 컴파일 단계에서 잡아주기 때문에, 개발자가 쉽게 파악할 수 있다.
- 또한 기존에 SQL과 jpql에서는 동적 쿼리를 작성하기가 어려운 문제가 있었다. 따라서, 이러한 문제를 해결하기 위해 JPA에서 Criteria API와 MetaModel Criteria API 를 만들었는데, 해당 기술은 너무 복잡하고, 배울 것도 너무 많고, 가장 중요한 부분이 알아보기 힘들다는 큰 문제가 있었다. 이러한 동적 쿼리에 대한 부분들도 QueryDSL이 해결해준다.

### DSL이란?
- DSL(Domain Specific Language) 라고 하는 도메인 특화 언어는 특정한 도메인에 초점을 맞춘 제한적인 표현력을 가진 컴퓨터 프로그래밍 언어이다. 이름에서 보면 알 수 있듯이 QueryDSL은 쿼리에 특화된 언어이다. 사실 QueryDSL은 Spring Data 처럼 어느 한 데이터베이스에 초점을 둔 것이 아닌, MongoDB, MySQL, JPA 등등 어떠한 도메인에서도 CRUD와 동적 쿼리에 대한 쿼리들을 추상화하기 위해서 제작됐다고 한다. 작동 방식은 QueryDSL -> JPQL -> SQL 순으로 변환되서 전송된다.
